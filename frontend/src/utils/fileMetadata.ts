...
export function inferTrackMetadata(name: string): InferredTrackMetadata {
  ...
  const tempoKeyMeta = extractTempoAndKey(normalizedName);
  const versionMeta = extractVersion(tempoKeyMeta.clean);
  const split = splitArtistAndTitle(versionMeta.clean);
  const fallbackTitle = versionMeta.clean || normalizedName || rawName;
