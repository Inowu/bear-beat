...
export const countriesRouter = t.router({
  aggregateCountries: publicProcedure
    .input(CountriesAggregateSchema).query(async ({ ctx, input }) => {
      const aggregateCountries = await ctx.prisma.countries.aggregate(input);
      return aggregateCountries;
    }),
  createManyCountries: publicProcedure
    .input(CountriesCreateManySchema).mutation(async ({ ctx, input }) => {
      const createManyCountries = await ctx.prisma.countries.createMany(input);
      return createManyCountries;
    }),
  createOneCountries: publicProcedure
    .input(CountriesCreateOneSchema).mutation(async ({ ctx, input }) => {
      const createOneCountries = await ctx.prisma.countries.create(input);
      return createOneCountries;
    }),
  deleteManyCountries: publicProcedure
    .input(CountriesDeleteManySchema).mutation(async ({ ctx, input }) => {
      const deleteManyCountries = await ctx.prisma.countries.deleteMany(input);
      return deleteManyCountries;
    }),
  deleteOneCountries: publicProcedure
    .input(CountriesDeleteOneSchema).mutation(async ({ ctx, input }) => {
      const deleteOneCountries = await ctx.prisma.countries.delete(input);
      return deleteOneCountries;
    }),
  findFirstCountries: publicProcedure
    .input(CountriesFindFirstSchema).query(async ({ ctx, input }) => {
      const findFirstCountries = await ctx.prisma.countries.findFirst(input);
      return findFirstCountries;
    }),
  findFirstCountriesOrThrow: publicProcedure
    .input(CountriesFindFirstSchema).query(async ({ ctx, input }) => {
      const findFirstCountriesOrThrow = await ctx.prisma.countries.findFirstOrThrow(input);
      return findFirstCountriesOrThrow;
    }),
  findManyCountries: publicProcedure
    .input(CountriesFindManySchema).query(async ({ ctx, input }) => {
      const findManyCountries = await ctx.prisma.countries.findMany(input);
      return findManyCountries;
    }),
  findUniqueCountries: publicProcedure
    .input(CountriesFindUniqueSchema).query(async ({ ctx, input }) => {
      const findUniqueCountries = await ctx.prisma.countries.findUnique(input);
      return findUniqueCountries;
    }),
  findUniqueCountriesOrThrow: publicProcedure
    .input(CountriesFindUniqueSchema).query(async ({ ctx, input }) => {
      const findUniqueCountriesOrThrow = await ctx.prisma.countries.findUniqueOrThrow(input);
      return findUniqueCountriesOrThrow;
    }),
  groupByCountries: publicProcedure
    .input(CountriesGroupBySchema).query(async ({ ctx, input }) => {
      const groupByCountries = await ctx.prisma.countries.groupBy({ where: input.where, orderBy: input.orderBy, by: input.by, having: input.having, take: input.take, skip: input.skip });
      return groupByCountries;
    }),
  updateManyCountries: publicProcedure
    .input(CountriesUpdateManySchema).mutation(async ({ ctx, input }) => {
      const updateManyCountries = await ctx.prisma.countries.updateMany(input);
      return updateManyCountries;
    }),
  updateOneCountries: publicProcedure
    .input(CountriesUpdateOneSchema).mutation(async ({ ctx, input }) => {
      const updateOneCountries = await ctx.prisma.countries.update(input);
      return updateOneCountries;
    }),
  upsertOneCountries: publicProcedure
    .input(CountriesUpsertSchema).mutation(async ({ ctx, input }) => {
