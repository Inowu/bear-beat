import { z } from 'zod';

import type { Prisma } from '@prisma/client';

const Schema: z.ZodType<Prisma.FtpQuotaLimitsCountAggregateInputType> = z
  .object({
    id: z.literal(true).optional(),
    name: z.literal(true).optional(),
    quota_type: z.literal(true).optional(),
    per_session: z.literal(true).optional(),
    limit_type: z.literal(true).optional(),
    bytes_in_avail: z.literal(true).optional(),
    bytes_out_avail: z.literal(true).optional(),
    bytes_xfer_avail: z.literal(true).optional(),
    files_in_avail: z.literal(true).optional(),
    files_out_avail: z.literal(true).optional(),
    files_xfer_avail: z.literal(true).optional(),
    _all: z.literal(true).optional(),
  })
  .strict();

export const FtpQuotaLimitsCountAggregateInputObjectSchema = Schema;
