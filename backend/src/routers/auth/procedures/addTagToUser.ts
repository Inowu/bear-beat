...
import { createSubscriber, getInfo, setCustomField_phone, updateSubscriber } from './manyChatApi';
...
export const addTagToUser = shieldedProcedure
    .input(
        ...
        async ({ ctx: { session, prisma }, input: { tagName } }) => {
            ...
            if (users?.id != null) {
                ...
                if(many_chat_id != null){
                    ...
                    prisma.users.update({where: { id: user.id }, data: { mc_id:  many_chat_id }})
                }
            } /*else {
